...
uint8_t NdefMessage::getRecordCount() { return _recordCount; }
...
bool NdefMessage::addRecord(const NdefRecord &record)
{
...
void NdefMessage::addTextRecord(const char *text) { addTextRecord(text, "en"); }
...
void NdefMessage::addExternalRecord(const char    *type,
                                    const byte    *payload,
                                    const uint16_t payloadLength)
{
  NdefRecord r;
...
NdefRecord NdefMessage::operator[](uint8_t index) { return getRecord(index); }
